...
class MultiAgentAnalyzer:
    ...
    async def analyze_tweet(self, tweet: Tweet, run_id: str) -> AnalysisResult:
        ...
        for i, agent_name in enumerate(self.agent_sequence, 1):
            self.logger.info(f"🔍 Executing {agent_name} ({i}/12)")
            ...
            try:
                ...
                # Execute agent
                response = await self._execute_agent(formatted_prompt)
...
""".strip()
    ...
    async def _execute_agent(self, prompt: str) -> str:
        """Execute individual agent using OpenAI API"""
        try:
